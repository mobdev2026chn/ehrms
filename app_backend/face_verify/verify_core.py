...
def embed_for_enroll(img):
    ...
    Enforces that exactly one face is present, the face is looking straight (not side angle),
    and is not covered/blocked by checking landmark completeness.
    ...
    # Enforce exactly one face in the resolved rotation
    locs = face_recognition.face_locations(rgb)
    if len(locs) > 1:
    ...
    # Frontal-pose (look straight) and occlusion checks
    top, right, bottom, left = loc
    ...
    if lm_list:
        ...
        # 1. Occlusion check: key features must be present
        has_left_eye = len(lm.get('left_eye', [])) > 0
        has_right_eye = len(lm.get('right_eye', [])) > 0
        has_nose = len(lm.get('nose_bridge', [])) > 0 or len(lm.get('nose_tip', [])) > 0
        has_mouth = len(lm.get('top_lip', [])) > 0 or len(lm.get('bottom_lip', [])) > 0
        if not (has_left_eye and has_right_eye and has_nose and has_mouth):
            return None, "Your face is partly blocked. Remove anything covering it and try again."

        # 2. Look straight (frontal pose check)
        ...
        if le and re:
            ...
            if total > 0:
                ...
                # Enroll is slightly more lenient than live punch (0.35-0.65 instead of 0.36-0.64)

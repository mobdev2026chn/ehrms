...
const createPermissionRequest = async (req, res) => {
    try {
        ...
        // Leave and Permission are mutually exclusive on the same day: if an
        // approved leave already covers the requested date (today OR any future
        // day), block the permission request. Matches the UTC day-bucket the
        // attendance/leave records use on the UTC server.
        {
            const reqD = new Date(date);
            const dayStart = new Date(Date.UTC(reqD.getUTCFullYear(), reqD.getUTCMonth(), reqD.getUTCDate(), 0, 0, 0, 0));
            const dayEnd = new Date(Date.UTC(reqD.getUTCFullYear(), reqD.getUTCMonth(), reqD.getUTCDate(), 23, 59, 59, 999));
            ...
            if (approvedLeave) {
                return res.status(400).json({ success: false, error: { message: 'You already have an approved leave on this day. Permission cannot be applied.' } });
            }
        }

        // One permission per type per day. The three types — Late Arrival,
        // Early Exit and Custom (`both`) — can be combined on the same day, but
        // the SAME type cannot be applied twice. A still-live request (Pending or
        // Approved) of this type on this day blocks a duplicate; Cancelled and
        // Rejected requests don't count, so the employee can re-apply after one.
        {
            const reqD = new Date(date);
            const dayStart = new Date(Date.UTC(reqD.getUTCFullYear(), reqD.getUTCMonth(), reqD.getUTCDate(), 0, 0, 0, 0));
            const dayEnd = new Date(Date.UTC(reqD.getUTCFullYear(), reqD.getUTCMonth(), reqD.getUTCDate(), 23, 59, 59, 999));
            ...
            if (existing) {
                const typeLabel = { lateArrival: 'Late Arrival', earlyExit: 'Early Exit', both: 'Custom' }[String(type)] || 'this';
                return res.status(400).json({ success: false, error: { message: `You already have a ${typeLabel} permission on this day. The same permission type cannot be applied twice.` } });
            }
        }
        ...
        //  - Today before punch-in: all types.
        //  - Today active session (punched in, not out): all except Late Arrival
        //    — it no longer applies once you've arrived (punched in).
        //  - Today after punch-out: none (the day is closed; the app moves the
        //    selection to a future date, where all types are allowed again).
        ...
        // Uses the same UTC day-bucket as the attendance punch log to match records.
        {
            ...
            if (isToday) {
                ...
                // Before punch-in: all types allowed. After punch-in: Late Arrival
                // no longer applies (you've already arrived).
...
const getPermissionBalance = async (req, res) => {
    try {
        ...
        // The three tiles are distinct buckets so the same minute is never counted
        // twice:
        //   - "Used"     = minutes actually utilized from Approved permissions
        //                  (consumedMinutes, computed above).
        //   - "Pending"  = minutes still AWAITING approval (status 'Pending' only).
        //                  Approved-but-not-yet-utilized minutes are NOT pending and
        //                  NOT yet used — they simply reduce the remaining allocation.
        // Group requested minutes by status so Pending reflects only what HR has yet
        // to decide on, while Approved still holds against the quota for `remaining`.
        const committedAgg = await PermissionRequest.aggregate([
            { $match: { employeeId, date: { $gte: start, $lte: end }, status: { $in: ['Pending', 'Approved'] } } },
            { $group: { _id: '$status', total: { $sum: { $ifNull: ['$requestedMinutes', 0] } } } }
        ]);
        const byStatus = Object.fromEntries((committedAgg || []).map(r => [r._id, Math.max(0, Number(r.total || 0))]));
        // Displayed "Pending" = only requests still awaiting approval.
        const pendingMinutes = byStatus['Pending'] || 0;
        const approvedMinutes = byStatus['Approved'] || 0;

        // Remaining must account for ALL committed hours (pending + approved) so the
        // employee cannot over-apply beyond their allocation — the approved-unused
        // portion is reserved even though it is not shown in any tile. consumedMinutes
        // is the utilized slice of `approvedMinutes`, so it is already covered by it.
        const reservedMinutes = pendingMinutes + Math.max(approvedMinutes, consumedMinutes);
        remainingMinutes = Math.max(0, monthlyQuotaMinutes - reservedMinutes);

...
function toUserFriendlyEnrollMessage(raw) {
    ...
    // Blurry, orientation, or positioning/occlusion capture problems — pass the engine's
    // actionable wording through verbatim so the user knows exactly what to correct.
    if (s.includes('straight') || s.includes('side angle') || s.includes('multiple faces')
        || s.includes('covered') || s.includes('block') || s.includes('blurry')) {
        return raw;
    }

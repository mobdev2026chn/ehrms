...
//                 if (isScreenshotTooSoon || (activityProcessor.isSkippableTrackingError && activityProcessor.isSkippableTrackingError(err))) {
...
//                 if (isScreenshotTooSoon || (activityProcessor.isSkippableTrackingError && activityProcessor.isSkippableTrackingError(fallbackErr))) {
//                     return res.status(200).json({ success: true, skipped: true });
//                 }
    ...
    try {
        ...
        if (!USE_REDIS) {
            ...
            try {
                ...
                const isScreenshotTooSoon = metadata?.type === 'screenshot' && /too soon/i.test(err.message || '');
                if (isScreenshotTooSoon || (activityProcessor.isSkippableTrackingError && activityProcessor.isSkippableTrackingError(err))) {
                    return res.status(200).json({ success: true, skipped: true });
                }
                console.error('Error (activity upload):', err.message || err);
